...
export const isBot = (userAgent: string | undefined) => {
  if (!userAgent) return false
  return botRegex.test(userAgent)
}

export const botName = (userAgent: string | undefined) => {
...
export const fromRequest = async (context: APIContext) => {
  ...
  const user_agent =
    ...
    undefined
  const bot = isBot(user_agent)
  const bot_name = bot ? botName(user_agent) : undefined
